...
import pandas as pd
from src.agent.nodes.ml_train import (
    normalize_columns,
...
def predict_node(state: dict) -> dict:
    ...
    # Step 1: Normalize + add same features as training
    test_df = normalize_columns(state["test_df"])
    test_df = add_landmark_distances(test_df)   # ← same landmarks as training
    ...
    # Step 3: Select same feature columns as training
    ...
    # Step 5: property_id — fallback to row index if missing
